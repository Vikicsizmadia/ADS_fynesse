...
def predict_prices(conn, year, property_type, postcodes):
  ...
  results_basis = train_model(conn, year, property_type, postcodes)
  ...
  design_pred = np.concatenate((X_avgPrice.reshape(-1,1), X_numShops.reshape(-1,1), X_numHealth.reshape(-1,1), X_numReligion.reshape(-1,1), X_numTransport.reshape(-1,1)),axis=1)
  Y_predPrice = results_basis.get_prediction(design_pred).summary_frame(alpha=0.05)
